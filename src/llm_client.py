...
def get_client(role: str = "agent"):
    ...
    Provider is chosen by FORCE_MOCK_LLM, LLM_PROVIDER (env), or auto-detected:
      - FORCE_MOCK_LLM=1         -> deterministic MockClient (overrides everything)
      - "ollama"                 -> OllamaClient (local, free, no API key; needs Ollama running)
      - "anthropic"              -> AnthropicClient (needs ANTHROPIC_API_KEY)
      - default (if key present) -> AnthropicClient, else MockClient
    """
    provider = _provider()
    if provider == "ollama":
